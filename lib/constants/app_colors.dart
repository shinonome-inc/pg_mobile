import 'dart:ui';

class AppColors {
  AppColors._();

  static const Color gray1 = Color(0xFF222222);
  static const Color gray2 = Color(0xFF353535);
  static const Color gray3 = Color(0xFF828282);
  static const Color gray4 = Color(0xFFBDBDBD);
  static const Color gray5 = Color(0xFFE0E4E7);
  static const Color gray6 = Color(0xFFF2F2F2);

  static const Color gray1A80 = Color(0xCC222222);
  static const Color gray5A80 = Color(0xCCE0E4E7);

  static const Color accent = Color(0xFF05A59D);
  static const Color deepAccent = Color(0xFF057D75);

  static const Color white = Color(0xFFFFFFFF);
  static const Color black = Color(0xFF000000);
  static const Color blue = Color(0xFF0081B9);
  static const Color yellow = Color(0xFFF2C94C);
  static const Color red = Color(0xFFEB5757);
  static const Color transparent = Color(0x00000000);
}
